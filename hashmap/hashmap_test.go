 ...
 * @LastEditTime: 2023-08-06 15:06:19
 */
package hashmap
...
func TestInit(t *testing.T) {
	mp := NewHashMap[string, int]()
	mp.Init([]Pair[string, int]{
		{
			Key:   "hello",
			Value: 1,
		},
...
func TestClear(t *testing.T) {
	...
	if tmp := NewHashMap[string, int](); !tmp.Empty() {
		t.Error("hashmap is empty")
	}
}
...
func TestForEach(t *testing.T) {
	...
	index := 0
	var Key0 string
	mp.ForEach(func(Key *string, Value *int) {
		if index == 0 {
